...
export default function DayTimelineView({
  ...
  const getRowByTimeString = (timeStr?: string, isEnd = false) => {
    if (!timeStr || timeSlots.length === 0) return 2;
    
    const [h, m] = timeStr.split(':').map(Number);
    const minutes = h * 60 + m;
    
    // timeSlotsの最初の時間を基準にする
  ...
  return (
    <div className="flex-1 overflow-auto bg-gray-50 p-2 md:p-6 mb-16 md:mb-0 relative">
      {timeSlots.length > 0 ? (
        ...
        <div className="flex items-center justify-center h-full text-gray-400 font-bold">当日タスクがありません</div>
      )}
    </div>
  );
}